...
quiz(caption = "Quiz - Install R",
  ...
  question("What is the current version of R?",
    answer("3.3.2", message = "It is at least version 4.0.3."),
    answer("3.6.2", message = "It is at least version 4.0.3."),
    answer("4.0.0", message = "It is at least version 4.0.3."),
    answer("4.0.3", correct = TRUE, "It was version 4.0.3 when I built this, but the current version might have a higher number (could be newer)."),
    answer("Other", message = "It was version 4.0.3 when I built this, but the current version might have a higher number (could be newer).")
  )
)
...
quiz(caption = "Quiz - Install RStudio",
  ...
  question("What is the current version of RStudio IDE?",
    answer("1.0.136", message = "It is at least version 1.3.1093."),
    answer("1.3.1093", correct = TRUE, "It was version 1.3.1093 when I built this, but the current version might have a higher number (could be newer)."),
    answer("4.0.3", message = "That was the current version of R, RStudio is at least version 1.3.1093."),
    answer("1.3", message = "It is at least version 1.3.1093."),
    answer("Other", message = "It was version 1.3.1093 when I built this, but the current version might have a higher number (could be newer).")
  )
)
```
...
As a backup to the preferred version of having a local installation of R and RStudio, Rstudio server go to https://rstudio.math.montana.edu and sign in using your NetID credentials. Currently you need to be on a list to be approved to access this server version of RStudio (hopefully related to how you ran into this tutorial) and so this method may not be available to you (especially if you are not MSU affiliated). In this version of RStudio, to load a data set or .Rmd file, go to the "Files" tab in the lower right panel (not "File" in the upper ribbon) and use the "Upload" button and "Choose File" to pick the file from your local directory structure. This same "Files" location can also be used to download files (use "More" and then "Export" to download .Rmd files). Working in the session window should otherwise resemble working on a local installation of RStudio and be running the current version (4.0.3). Your work should be available when you re-open this until the end of the semester, but because it is new, we would recommend downloading your work at the end of each session until we can test the system more fully. Note that this version of access is a temporary solution and not a long-term way to use R.
