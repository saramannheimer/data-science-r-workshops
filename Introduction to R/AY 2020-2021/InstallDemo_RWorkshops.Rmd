...
quiz(caption = "Quiz - Install R",
  ...
  question("What is the current version of R?",
    answer("3.3.2", message = "It is at least version 4.0.2."),
    answer("3.6.2", message = "It is at least version 4.0.2."),
    answer("4.0.0", message = "It is at least version 4.0.2."),
    answer("4.0.2", correct = TRUE, "It was version 4.0.2 when I built this, but the current version might have a higher number (could be newer)."),
    answer("Other", message = "It was version 4.0.2 when I built this, but the current version might have a higher number (could be newer).")
  )
)
...
quiz(caption = "Quiz - Install RStudio",
  ...
  question("What is the current version of RStudio IDE?",
    answer("1.0.136", message = "It is at least version 1.3.1056."),
    answer("1.3.1056", correct = TRUE, "It was version 1.3.1056 when I built this, but the current version might have a higher number (could be newer)."),
    answer("4.0.2", message = "That was the current version of R, RStudio is at least version 1.3.1056."),
    answer("1.3", message = "It is at least version 1.3.1056."),
    answer("Other", message = "It was version 1.3.1056 when I built this, but the current version might have a higher number (could be newer).")
  )
)
```
...
As a backup to the preferred version of having a local installation of R and RStudio, we have an option to use a virtual windows machine that is hosted by MSU - but only if you are an MSU affiliate. We will try to have it available to you based on your NETID. If you need this option, please go to  [https://studentlabs.montana.edu/remotelabs/howto.html](https://studentlabs.montana.edu/remotelabs/howto.html), click on "Connect to virtual machine via a web browser", then follow the directions to login and use it at "remotelabs.msu.montana.edu". You should find a "CLS-STAT-REMOTE" box on the page with "Horizon". That build has a new install of R and RStudio and would operate the same way as having a local installation, but via the web browser. Note that files are not saved unless you save them in Onedrive or some other cloud service (Box, google drive, etc.). Because this is a streaming workshop, it might tax your internet connection to both stream the workshop and use this virtual desktop and so we recommend using a local installation of RStudio if at all possible.
