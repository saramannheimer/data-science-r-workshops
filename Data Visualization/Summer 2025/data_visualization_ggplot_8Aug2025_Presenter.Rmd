...
subtitle: "Data Science Workshop - Session 2"
author: "Data Carpentry contributors & Montana State University R Workshops Team"
output: pdf_document
header-includes: |
  \usepackage{fancyhdr}
  \pagestyle{fancy}
  \fancyfoot[LO,RE]{Session 2: Data Visualization - Presenter}
...
It is important to note that there's no need to re-install packages every time 
we run the script.
...
session. We will proceed through the remaining work with the `tidyverse` package
...
package **`readr`**, instead of `read.csv()` used in the previous session.
...
we will discuss in the _Data Wrangling in `R`_ session, as well as how to 
...
```{r challenge-6-example, fig.height=3, fig.width=6}
surveys %>% 
  ggplot(mapping = aes(x = species_id, color = species_id)) + 
  geom_bar()
```
