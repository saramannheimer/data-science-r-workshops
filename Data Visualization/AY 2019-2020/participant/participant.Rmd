...
* a "." to draw the smallest rectangle that is visible -- typically 1 pixel
...
Therefore, we can be lazy and leave the argument unnamed, and `R` will still 
...
`ggplot()` function. This will be seen by any geom layers and the mapping will 
be determined by the x- and y-axis set up in `aes()`.
...
I was once told that they idea behind a good histogram was to make the plot look 
...
**Why do we need to set `group = 1` in the above proportion bar chart? In other 
words, what is wrong with the plot below?**   
