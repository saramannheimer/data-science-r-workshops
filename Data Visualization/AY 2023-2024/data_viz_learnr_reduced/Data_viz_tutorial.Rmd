...
*  These workshops are taught by Greta Linse and Sara Mannheimer and co-organized by the MSU Library, Statistical Consulting and Research Services (SCRS), and the Department of Mathematical Sciences. More details on us and other workshops are available at the end of the session or via https://www.montana.edu/datascience/training/#workshop-recordings.

*  A PDF version of this Workshop from a previous year (only minor differences) is available at: [Data Viz R Fall 2022](https://montanaedu-my.sharepoint.com/:f:/g/personal/w83j978_msu_montana_edu/EnB37Khlrr9Okmbpgq6ysgsBxYJzuCfGhDJadWmzRkffLw?e=V4XKxh "Data Viz R Fall 2022 PDF")
...
should import it in every subsequent `R` session when you need it. If you were to 
do this work in RStudio, you would need to install the **`tidyverse`** package. This is an
...
In this workshop, we have already installed the `tidyverse` using 
`install.packages("tidyverse")`. It is important to note that there's no need 
to re-install packages every time we run the script.
...
Working with packages was discussed in more detail in the "Introduction to R" 
workshop. We will proceed through the remaining work with the `tidyverse` package 
...
```{r load-data, message = FALSE, exercise=TRUE}
surveys <- read_csv("https://raw.githubusercontent.com/saramannheimer/data-science-r-workshops/master/Data%20Visualization/AY%202020-2021/data/surveys2.csv")
```
...
```{r inspect-data, message=TRUE, exercise=TRUE}
## inspect the data
str(surveys)
...
>**Note:** A "filled" shape means that it is filled with another color or is 
>"open" if no fill color is specified. A solid shape has no further 
>specification.
...
>**Note:** There is now (as of R 4.3.0) a Base R pipe `|>` that works similar to the 
>`%>%` pipe operator with minor differences that you may encounter, but since
>we are working in the `tidyverse` we will stick with their pipe operator (`%>%`).
...
Notice that we can change the geom layer and colors will still be determined by
**`species_id`**
...
> `plot_id` with data from different plot types being shown in different 
...
Boxplots provide a visualization of a quantitative variable across different levels of a categorical (grouping) variable. For example, we can use boxplots to visualize the distribution of weight within each species:
...
> change in the code to put the boxplot in front of the points? (We'll come back to this...)
...
There is no single right answer for the number of bins. There are some "plug-in" choices for the number of bins that can be used, but you are always welcome to explore different numbers of bins to see if features you are seeing persist when you choose more or less bins.  
...
```{r challenge-6, exercise=TRUE, exercise.lines = 7}
...
Let's calculate the number of counts per year for each genus. 
...
Both geometries allow you to specify faceting variables using formula notation or the
`vars()` function. We will use the formula notation, as you will use this notation
when creating models (not covered in this workshop).
...
```{r facet4, fig.height = 8.5, fig.width = 8, exercise = TRUE}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
...
```{r facet5, fig.width = 9.5, fig.height = 5, exercise = TRUE}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
...
```{r facet6, fig.width = 9.5, exercise=TRUE}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
...
```{r challenge-7, exercise=TRUE, exercise.lines = 13}
## To get you started:  
yearly_species_weight <- surveys %>% 
  group_by(year, species_id) %>%
  ## Variables to group by
...
Usually plots with white backgrounds look more readable when printed. 
...
```{r challenge-8, exercise=TRUE, exercise.lines = 7}
## Your ggplot() code for the plot goes here!
...
Take a look at the [**`ggplot2`** cheat sheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf), 
and think of ways you could improve the previous plot.

### Plot Labels 

Now, let's change the names of axes to something more informative than 'year'
...
Install a package and use its available color scales. Popular options 
include:  
...
> beautiful graph of your own. Use the RStudio [**`ggplot2`** cheat sheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf)
...
using different variables or even different data frames. 
...
```{r arrange, message = FALSE, fig.width = 10, fig.height=6, exercise = TRUE}
library(gridExtra)
...
- `width` and `height`: adjust the total plot size in units ("in", "cm", or "mm")
  * If units are not specified, the default is inches. 
...
```{r plotlydemo, message = FALSE, exercise = TRUE, exercise.eval=TRUE}
spp_weight_boxplot <- surveys %>% 
  ggplot(aes(x = genus, y = weight)) +
  geom_violin() +
  geom_jitter(color = "tomato", width = 0.2, alpha = 0.2) + 
...
The workshops for 2023-2024 involve modifications of materials and are licensed CC-BY. ![Creative Commons License](88x31.png) This work is licensed under a [Creative Commons Attribution 4.0 International License](http://creativecommons.org/licenses/by/4.0/).
