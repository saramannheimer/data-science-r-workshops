...
surveys %>% 
  ggplot(mapping = aes(xx = plot_id, y = weight)) +
  geom_jitter(aes(color = plot_type), alpha = 0.1) + 
  geom_smooth()
...
```{r datasaurus, message = FALSE, exercise = TRUE}
library(datasauRus)
data(box_plots)
...
box_plots_long <- box_plots_long %>%
  rename(Example=key, Response=value) 
