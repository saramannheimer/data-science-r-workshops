...
```{r preview-data, exercise=TRUE}
## Preview the data: View(surveys) #Or
glimpse(surveys)
```

You could also use `View(surveys)` or `glimpse(surveys)` when working in RStudio to explore the data set further.
...
as smooth as possible -- think about having it resemble the continuous shape of the 
density plot.  
...
> Play around with which variable you facet by versus plot by! (Note that you need to scroll down in the sandbox to add code to complete this challenge.) 
...
```{r export, exercise=TRUE, eval = FALSE}
...
## Tips for Visually Appealing Data Viz
...
- Colors are hard, so think about them carefully! Remember to think about color-blind viewers.
