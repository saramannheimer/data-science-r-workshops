...
```{r challenge-2part1-solution, include=FALSE}
surveys %>% 
  ggplot(mapping = aes(x = weight, y = hindfoot_length)) +
...
```{r challenge-2part2-solution, include=FALSE}
surveys %>% 
...
```{r challenge-3part1-solution, include=FALSE}
surveys %>% 
  ggplot(mapping = aes(x = species_id, y = weight)) +
  geom_violin() + 
...
```{r challenge-3part2-solution, include=FALSE}
surveys %>% 
...
```{r challenge-3part3-solution, include=FALSE}
surveys %>% 
  ggplot(mapping = aes(x = species_id, y = hindfoot_length)) +
...
```{r challenge-7-solution, include=FALSE}
## To get you started:  
yearly_species_weight <- surveys %>% 
  ## Variables to group by:
  group_by(year, species_id) %>%
...
```{r staticPlotly1, message=FALSE, echo=FALSE}
spp_weight_boxplot + theme_bw()
```

```{r plotlydemo2, message = FALSE, eval=FALSE}
spp_count_plot <- yearly_counts %>% 
  ggplot(aes(x = year, y = n, color = genus)) +
  geom_line() + 
  labs(x = "Year", 
...
```{r  staticPlotly2, message=FALSE, echo=FALSE}
spp_count_plot + theme_bw()
```
