...
3. Add "geoms" -- graphical representations of the data in the plot (points, lines, bars). **`ggplot2`** offers many different geoms; we will use some 
...
```{r plus-sign, echo=TRUE, eval=FALSE}
...
```{r challenge-7, echo=TRUE, eval=FALSE}
## To get you started:  
...
```{r labels, fig.height=6,fig.width=7}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_wrap(vars(species_id)) +
...
```{r fonts, fig.height=6,fig.width=7}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_wrap(vars(species_id)) +
...
**Note:**  `theme_bw()` is a function for a **specific** theme and `theme()` is a
...
```{r flip, fig.height=6,fig.width=7}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_wrap(vars(species_id)) +
...
```{r text-size, fig.height=6,fig.width=7}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_wrap(vars(species_id)) +
...
```{r legend-pos, fig.height=6,fig.width=7}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_wrap(vars(species_id)) +
...
```{r remove-grid, fig.height=6,fig.width=7}
yearly_sex_counts %>% 
  ggplot(mapping = aes(x = year, y = n, color = sex)) +
  geom_line() +
  facet_wrap(vars(species_id)) +
