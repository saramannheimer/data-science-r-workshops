...
**Location:** Montana State University, TBD - with an online option  
...
list, these can be variables, parameters, or other information. This will help you understand the structure of data.frames (coming up later)!
...
```{r challenge-s2-6part1-example}
BlackfootFish_clean |>
  ggplot(mapping = aes(x = species, y = weight)) +
  geom_jitter(alpha = 0.2, color = "tomato")
```

```{r challenge-s2-6part1}
##  Start with the boxplot we created
...
```{r challenge-s2-9-example, fig.height=3, fig.width=6}
BlackfootFish_clean |>
  ggplot(mapping = aes(x = species, color = species)) + 
  geom_bar()
```
