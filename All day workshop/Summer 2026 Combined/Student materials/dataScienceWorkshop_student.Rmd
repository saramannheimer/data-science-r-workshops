...
"BlackfootFish.CSV" file or open this file in Excel.
...
uses a dataset as its only parameter. This is how that function would work when
using piping.
...
Modify the following code to select the `length`, `weight`, and `species` columns from the `BlackfootFish` dataset:
...
Modify the following code to select all columns *except* `trip` and `mark`:
...
This will select all the variables in `BlackfootFish` except `trip`
and `mark`.
