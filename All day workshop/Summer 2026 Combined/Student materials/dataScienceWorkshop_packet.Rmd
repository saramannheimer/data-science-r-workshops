...
documentclass: book
description: "Course materials for the all-day data science workshop where you'll
...
## Data Science Workshop, August 14, 9am - 5pm {-}
...
**Location:** Montana State University, American Indian Hall (AIH) Room 166 - with an online option  
**Date:** Friday, August 14, 2026  
**Time:** 9 a.m. - 5 p.m.  

### Introduction to `R` - 9-10:30am  {-}
...
Learn the basics of `R`, RStudio, and the Tidyverse.

### Break - 10:30-10:45am {-}

### Introduction to `R` Continued - 10:45-11:30am {-}

### Q&A - 11:30-Noon {-}

### Lunch (provided) - 12-1:00pm {-}

### Data Visualization in `R` - 1-2:30pm  {-}
...
Learn to visualize data in `R` using the ggplot2 package.

### Break - 2:30-2:45pm {-}

### Coding with Generative AI - 2:45-4:30pm  {-}
...
>
> * Understand the RStudio user interface and know where to change default settings.
> * Know how to create or open an `R` package.
> * Know how to open an RMD file.
> * Know how to search for help in RStudio.
> * Know how to use the command line to use `R` as a calculator.
...
For this workshop, we will be making use of RStudio which you will need to 
install on your computer. 

1.  First you will want to go to https://cran.r-project.org/ 
to download for your specific operating system, run the `R` installer that you 
downloaded and  
1.  then go to https://posit.co/download/rstudio-desktop/ to download 
and install RStudio.
...
macOS. Unfortunately, a Chromebook, iPad, Snapdragon, or other ARM type computers
are not sufficient for installing and running R. If you have borrowed a computer
from the library you will also not be able to install the necessary software.

**NOTE:** If you are unable to install the software there are online apps that
allow you to run RStudio. You can try https://webr.sh to see if that works for you.

However you get to it, RStudio has four default panels, where each can be viewed at the 
same time and has multiple tabs available. The location of these tabs can be
adjusted in the **View -> Panes -> Panes Layout...** section of the **Global Options**
dialog, it is possible that your initial layout may be different than what is shown
below. The content and function remains the same. (See Figure 1)
...
many consider these the best way to work in `R`. A motivation for using projects 
is that it is important to organize your data
...
You can either create a project in a new folder, or the existing folder containing
the workshop materials.

### Challenge 1:

Go ahead and create your first project! Create it in the **same directory** as the
workshop files.
...
of the code chunks (like we are doing here). 
...
demonstrate this at the end of the workshop - but this will only work if all 
...
flagging code and variable names. 
...
created during your `R` session. The value of `year` appears next to it, since it 
...
right to objects on the left. So, after executing `year <- 2026`, the value of 
`year` is `2026`. The arrow can be read as "`2026` goes into `year`". For 
historical reasons,
...
## Order matters and checking values

When you assign a value to an object `R` does not output
...
Once the object `year` has been created, you can use it! Run the following lines of code:
...
below create a new object `another_year` and then we give `year` a new value of 2030. 
...
What is the current value of `another_year`? 2032 or 2036? 
...
To give context to the workshop we are going to put ourselves in the role of a
biologist or ecologist studying the health and abundance of fish in the Blackfoot
river. We've already done the hard work of going out and collecting the data, now
...
1.  Summarize the health of the fish caught in this project in tables and graphs by
year caught, length, weight, and species.

Inferential questions include:

1.  The mean length of a Westslope Cutthroat Trout is different in different sections
of the Blackfoot River.

## Examine collected data

Navigate to where you have downloaded the files for today and double click on the
"BlackfootFish.CSV" file or open this file in Excel. See Figure 3.

**CAUTION:** For some types of data, including long text in cells and dates, opening
the file in Excel can modify the data in unexpected ways. You can fix those problems
after importing into `R`, but it is something to be aware of and check before
analyzing data.

***Advanced:*** Import this file into RStudio and explore the data.

We will keep these research questions and idea of the data in mind as we work through
answering the questions in `R` using RStudio.

![Blackfoot Fish data in Excel](images/BlackfootFish_in_Excel.png)

## Packages  

`R` has many packages, which people around the world
...
We're going to install a packages that is actually a collection of packages that are often used.  

* Use the __Install__ button in the __Packages__ tab  
* Type in `tidyverse` into the blank line  
...
Now that the files are downloaded, we need to load the package in order to use it. 
The following code will load all of the packages in the "tidyverse", please run it!  
...
Do not be intimidated by the red color or by the "conflicts". The red color is a feature of the theme in use by RStudio. The "conflicts" just indicate packages that have the same function name, and which package's function will be used when called. 

Here, `dplyr`'s `filter()` will be used instead of the filtering function in the `stats` package.
...
>
> ***ERROR ALERT*** If you ever get an error updating a package, do the following:
>
> 1.  Take a note of the name,
> 1.  navigate to where your packages are saved,
> 1.  close RStudio,
> 1.  delete the folder for that package,
> 1.  restart RStudio, and
> 1.  install that package again.
> 
...
* Choose the __From Text (readr)__ option  
...
>
> ***ERROR ALERT*** At this point, if you are having any errors, it is likely due
to working in a zipped folder. Fix this by doing the following:
> 
> 1.  In a file browser, navigate to where you downloaded the workshop materials,
> 1.  right click on the folder and extract to a new location such as your desktop,
> 1.  open the RMD file in the extracted folder by double clicking on it, it should 
open in RStudio.
> 
...
inspect the structure of the `BlackfootFish` object using functions.  
...
What is a dataframe? A dataframe (also called a "tibble" when created by the 
`tidyverse` package) is a type of `R` object and is the *de facto* 
...
which is one of the packages imported by the `tidyverse` package),
`read_excel` (requires `readxl` package), etc.). A dataframe is a 
...
For example, `c(1, 2)` could identify the first two objects in a list. However 
they are adjacent so we could use `a:b` to indicate that we want everything from 
position `a` to position `b` in the list. So, `c(1,2)` would give the same items 
as `c(1:2)`.

#### Quick Check

Run the following code to see what you get. Play around with the first and last 
...
When you want adjacent items, using the `a:b` notation is nicer than having to 
type out all of the values.
...
df[,c("x","y")]
df |> select(x,y) # alternate solution with piping

# Solution 2:
df[,c("x","z")]
df |> select(x,z) # alternate solution with piping
```
...
**You get an error:** "Error in s[3, ] : incorrect number of dimensions"

This is because there is only one dimension for this object and the comma in the square brackets implies multiple dimensions.


### Piping Data In  

Most functions require data to operate on. Because of this, the first argument to 
many functions is the dataset you wish to be working with. The pipe 
...
Pipes in `R` look like `|>`, there is also a pipe that looks like `%>%` from the **`magrittr`** 
package, installed automatically with the **`tidyverse`**. If you use RStudio, 
you can type the pipe with `Ctrl + Shift + M` if
you have a PC, or `Cmd + Shift + M` if you have a Mac. In the settings 
(Tools > Global Options > Code) you can change the pipe operator that is inserted
when you use those short cuts. 

**NOTE:** Generally native operators are preferred over
those that require packages, although there are many more piping operators available
in the `magrittr` package for those that are interested in digging deeper.

One function we used previously was the `summary()` function. This function
uses a dataset as its only parameter. This is how to call that function when
using piping to pass in the dataset.
...
We're going to learn some of the most common functions used to wrangle data:
...
to this function is the data frame (`BlackfootFish`), and the subsequent
arguments are the columns to keep.

The following code selects the `length`, `weight`, and `species` columns from the `BlackfootFish` dataset:

```{r select-columns}
BlackfootFish |>
  select(length, weight, species)
```

To select all columns *except* certain ones, put a "-" in front of
the variable to exclude it.

The following code selects all columns *except* `trip` and `mark`:
...
In the code above `==` keeps all rows where the year is 1998. 

Other filtering options include `!=`, which keeps all rows that are **not** a 
certain criteria, `,` which means **"and"**, and `|` which means **"or"**. 
Filter can also do `<` for "less than", `>` for "greater than", `<=` for "less 
than or equal to", and `>=` for "greater than or equal to". We type these last 
two options the same way we would typically say them.
...
2.  `,` example:
    ...
    BlackfootFish |>
      filter(year==1998, species=="WCT") # can also use "&" instead of a comma
    ```
    
    The code above keeps all rows where the year is 1998 for Westslope Cutthroat Trout, i.e., year 1998 and species WCT. The rows meet **both** of these criteria. 
...
3.  `|` example:
    ...
    ```{r filter-or-example}
    BlackfootFish |>
      filter(year==1998 | species=="WCT")
    ```
    
    The code above keeps all rows where the year is 1998 **or** is a Westslope Cutthroat Trout, i.e., year 1998 or species WCT. The rows meet **either** of these criteria but do not have to meet not both. 
...
### Perform multiple functions on the same dataset
...
As a refresher, in the above code, we use the pipe to send the `BlackfootFish` dataset
first through `filter()` to keep rows where `weight` is less than 200, then 
through `select()` to keep only the `species`, `length`, and `weight` columns.
...
an argument to the `filter()` and `select()` functions.

Some may find it helpful to read the pipe like the word "then". For instance,
in the above example, we took the data frame `BlackfootFish`, *then* we `filter`ed
for rows with `weight < 200`, *then* we `select`ed columns `species`, `length`,
and `weight`. The functions by themselves are somewhat simple,
...
BlackfootFish_sml <- BlackfootFish |>
  filter(weight < 200) |>
  select(species, length, weight)
...
>
>  Using pipes, subset the `BlackfootFish` data to include:
>
  -  fish collected on or after 2002 and 
  -  retain only the columns `year`, `species`, `length`, and `weight`.
>

```{r challenge-s4-2}
## Pipes Challenge:
##  Using pipes, subset the data to include animals collected
##  on or after 2002, and retain the columns `year`, `species`, `length`, and `weight`

BlackfootFish_challenge2 <- BlackfootFish |>
  filter(year >= 2002) |>
...
can use a pipe to view the `head()` of the data.
...
The dataset contains some missing observations (`NA`s). If we wanted to remove 
any observations where there were missing values on `weight`, we could insert a 
`filter()` in the chain:
...
>
>  Create a new data frame from the `BlackfootFish` data named `BlackfootFish_length_cm` 
> that meets the following criteria:
>
-  contains only the `species` column and 
-  a new column called `length_cm` containing the `length` values 
   converted to centimeters (they are in mm).
-  Make sure that you only retain values in the length_cm column that are not missing (not NA).
...
##  * Make sure that you only retain values in the length_cm column that are
##    not missing (not NA).
##  Then print out the head of the new data frame.

##  Hint: think about how the commands should be ordered to produce this data frame!
BlackfootFish_length_cm <- BlackfootFish |>
...
BlackfootFish_mod <- BlackfootFish |>
  mutate(speciesF = factor(species))
...
```{r}
unique(BlackfootFish_mod$section) ## tells you the unique values of section

BlackfootFish_mod <- BlackfootFish_mod |>
  mutate(sectionF = factor(section))
...
```{r, tidy = FALSE}
levels(BlackfootFish_mod$speciesF)
...
```{r}
class(BlackfootFish_mod$yearF)
levels(BlackfootFish_mod$yearF)
...
```{r}
year_recover <- as.numeric(BlackfootFish_mod$yearF)

ds <- data.frame(original = BlackfootFish_mod$yearF, 
...
`BlackfootFish` data (`str`, `class`, `summary`). 

A function transforms an input (potentially multiple) into an output. You have 
to provide `R` with the inputs (arguments) required for the function to generate
an output. The argument(s) inside a function happen after the `(` symbol. You can
determine that an object is a function when it is immediately followed by a  `(` 
and the corresponding closing `)` comes after the arguments are complete. The 
output of a function does not have to be numerical and it typically is not a 
single number, it can be a set of things or a dataset.   
...
`na.rm` as an option to remove `NA`'s *within* a function or for a particular 
variable, but the `na.omit` function takes a dataframe and removes any rows
that contain at least one `NA` for any variable from that dataset. 
Based on the output below, how many rows in the `BlackfootFish` data have an 
`NA` present?  
...
## with incomplete cases removed - incomplete cases (rows) are ones with at 
## least one NA in it
```
...
```{r}
BlackfootFish_clean <- na.omit(BlackfootFish_mod) 
## Creates a new dataframe, where the NA's have all been removed 

## Can also use piping
BlackfootFish_clean <- BlackfootFish_mod |>
  na.omit() 
```
...
When grouping both by `species` and `section`, if there were any fish missing either a species or a section, perhaps if a fish
escaped before their species could be determined, or the person recording forgot to write down the section of river... You would the last column would contain `NaN` (which refers to "Not a
Number"). While this does not matter for this data, to avoid this, we can remove the missing values for weight before we
...
Once the data are grouped, you can also summarize a variable multiple ways at the same
time (and not necessarily on the same variable). For instance, we could add a
column indicating the minimum weight for each species for each section:
...
It is sometimes useful to rearrange the result of a summary to inspect the values.
...
> length for each species. Also add the number of observations.
> (HINT: see `?n`.)
>
...
>
> What was the heaviest species of fish measured in each year for each section?
>

```{r challenge-s1-4-2}
BlackfootFish |>
  filter(!is.na(weight)) |>
  group_by(species, section, year) |>
  summarize(mean_weight = mean(weight),
            min_weight = min(weight)) |>
  arrange(desc(mean_weight))
```
...
for each categorical variable or group of categorical variables. For this task, **`dplyr`** provides
`count()`. For example, if we wanted to count the number of rows of data for
each section, we would do:
...
group. In other words, `BlackfootFish_clean |> count()` is equivalent to:  
...
for *one* factor (i.e., `section`). 
If we wanted to count the *combination of factors*, such as `species` and `section`, 
...
From the table above, we may learn that, for instance, there are 793 observations
of the Brown Trout species (`species` = "Brown") for fish caught in the Johnsrud section.
...
>
> How many fish were caught in each year of the survey?
> HINT: You will need to use the "BlackfootFish_mod" data set
> and the `yearF` variable.
>

```{r challenge-s1-5}
## Count Challenge:
##  How many fish were caught in each section for each year surveyed?
BlackfootFish_mod |>
  count(yearF)
```
...
After you have modified your data, you may wish to share it with
a collaborator or save it for future use in `R` or a different
program.

Typically, the most effective way to save a dataframe is the
nearly the same as how you read in the data, using the `write_csv()` function. Exceptions will be
discussed below.

The `write_csv()` function has three important parameters:

-  The dataset to save,
-  The file path to use, and
-  How NA values should be indicated (by default it is the string "NA").

```{r}
write_csv(BlackfootFish_mod, # dataframe name
          "BlackfootFish_mod.csv", # path, can just be the file name if you are using projects,
          na="NA" # stores missing values with "NA" (default)
          )
```

If you wanted to do analysis in another program, such as in Excel,
directly, you will want the missing values to be just empty cells.
In the code below the value of missing values has been set to be
the empty string.

```{r}
write_csv(BlackfootFish_mod, # dataframe name
          "BlackfootFis_mod.csv", # path, can just be the file name if you are using projects,
          na="" # stores missing values with empty cells
          )
```

If you have long strings or dates that you want Excel to handle
properly, you can also create an Excel workbook using the
`writexl` package (there are others such as `openxlsx2` that can
create Excel Workbooks, but `writexl` is one of the simplest
packages to use).
...
library(writexl)
write_xlsx(BlackfootFish_mod, # dataframe name
          "BlackfootFis_mod.xlsx" # path, can just be the file name if you are using projects,
          )
```
...
we have discussed in the _Data Wrangling in `R`_ workshop (we will share the
link later), as well as how to 
...
the `data` argument
    ...
    ```{r bind-plot}
    ggplot(data = BlackfootFish_clean)
    
    ## Creates a blank ggplot(), referencing the BlackfootFish_clean dataset
    ```
...
3. Add "geoms" -- graphical representations of the data in the plot (points,
  ...
  common ones today, including:
    ...
    * `geom_line()` for trend lines, time series, etc.
    ...
    ```{r add-to-plots, fig.width=3,fig.height=1.5}
    ggplot(data = BlackfootFish_clean, 
           mapping = aes(x = weight, y = length)) +
      geom_point()
...
```{r plus-sign, error=TRUE, fig.width=3,fig.height=1.5}
...
ggplot(data = BlackfootFish_clean, 
           mapping = aes(x = weight, y = length, shape=species)) +
    geom_point(alpha = 0.2, color = "blue")
```
...
BlackfootFish_clean |>  
  ggplot(mapping = aes(x = weight, y = length, color = species)) +
    geom_point(alpha = 0.2)
```
...
```{r challenge-s2-7part1}
## Your ggplot() code for the challenge goes here!

BlackfootFish_clean |>  
  ggplot(mapping = aes(x = weight, y = length)) +
  geom_jitter(aes(color = species, shape=section), alpha = 0.2) + 
  geom_smooth()
  ## one smoothing line -- no color defined globally

BlackfootFish_clean |>  
  ggplot(mapping = aes(x = weight, y = length)) +
  geom_jitter(aes(color = year), alpha = 0.2) + 
  geom_smooth()
```
...
> `year` with data from different years being showed in different 
> colors. Is this a good way to show this type of data?
>

```{r challenge-s2-7part2}
## Your ggplot() code for the challenge goes here!

BlackfootFish_clean |>  
  ggplot(mapping = aes(x = year, y = weight)) +
  geom_jitter(aes(color = yearF), alpha = 0.2) + 
  geom_smooth()
```
...
```{r challenge-s2-8part1-example, fig.width=4,fig.height=2}
BlackfootFish_clean |>
  ggplot(mapping = aes(x = species, y = weight)) +
  geom_jitter(alpha = 0.2, color = "tomato")
```

```{r challenge-s2-8part1, fig.width=4,fig.height=2}
##  Start with the boxplot we created
...
> species.
>
> Create a boxplot for `length`. This time overlay the boxplot layer 
> over a jitter layer that shows the actual measurements.
>

```{r challenge-s2-8part2, fig.height=3.5}
##  First: create boxplot for length` overlaid on a jitter layer.
BlackfootFish_clean |>
...
> the fish was captured on (`trip`).
>
> *Hint:* Check the class for `trip`. If `trip` was a character instead, 
> how would the graph be different?  
>

```{r challenge-s2-8part3, fig.height=3.5}
...
## how would the graph be different? 
BlackfootFish_clean |>
  ggplot(mapping = aes(x = species, y = length, color=factor(trip))) +
    geom_boxplot(alpha = 0) + 
    ## alpha = 0 eliminates the black (possible outlier) points, so they're not plotted twice
    geom_jitter(alpha = 0.03, position=position_jitterdodge())
    ## alpha = 0.2 decreases the opacity of the points, to not be too busy
```
...
if(!("datasauRus" %in% rownames(installed.packages()))){install.packages("datasauRus")}
...
```{r}
hist(BlackfootFish_clean$weight)
```
...
The x-axis displays the levels of `species`, a variable in the 
`BlackfootFish_clean` dataset. On the y-axis `count` is displayed, but `count` is 
...
(rows) associated with each level of `species`  
4. `geom_bar` uses this summary data frame, to build the plot -- levels of
`species` are plotted on the x-axis and `count` is plotted on the y-axis  
...
Each category shows 100% instead of the proportion of the fish in each category.
...
BlackfootFish_clean |>
  ggplot(mapping = aes(x = species, color = species)) + 
  geom_bar()
```
...
```{r challenge-s2-11-example, fig.height=3, fig.width=6}
BlackfootFish_clean |>
  ggplot(mapping = aes(x = species, color = species)) + 
  geom_bar()
```
...
Now suppose you are interested in whether the number of fish captured in different sections of the river differs by species. This would require for you to create a bar plot with 
two categorical variables. You have two options:  

1. each of the bars for section could be stacked within a species *OR* 
2. the bars for section could be side-by-side within a species  
...
```{r stacking}
BlackfootFish_clean |>
  ggplot(mapping = aes(x = species, fill = section)) + 
  geom_bar()
...
```{r timeseries1}
yearly_counts <- BlackfootFish_clean |>
  count(year, species) 
  ## counts the number of observations (rows) for each year, species combination 
...
```{r timeseries3}
yearly_counts |>
  ggplot(mapping = aes(x = year, y = n, group = species)) +
    geom_line()
```

Unfortunately, we can't tell what line corresponds to which species. We will be 
able to distinguish species in the plot if we add colors (using `color` also 
automatically groups the data):

```{r timeseries4}
yearly_counts |>
ggplot(mapping = aes(x = year, y = n, color = species)) +
    geom_line()
```
...
Now we would like to split the line in each plot by the section of the river the 
fish was captured in. To do that we need to make counts in the data frame grouped by `year`,
`species`, and `section`:

```{r facet2}
yearly_section_counts <- BlackfootFish_clean |>
  count(year, species, section)
## counts the number of observations (rows) for each year, species, section combination

yearly_section_counts
```

We can now make the faceted plot by splitting further by section using `color` 
(within each panel):

```{r facet3, fig.height=4, fig.width=6}
yearly_section_counts |>
  ggplot(mapping = aes(x = year, y = n, color = section)) +
    geom_line() +
    facet_wrap(facets =  ~ species)
```
...
yearly_section_counts |>
  ggplot(mapping = aes(x = year, y = n, color = section)) +
    geom_line() +
    facet_wrap(facets =  ~ species, ncol = 1)
  # One column, facet by rows
```

```{r facet5, fig.width = 7}
yearly_section_counts |>
  ggplot(mapping = aes(x = year, y = n, color = section)) +
    geom_line() +
...
```{r challenge-s2-12, error=TRUE}
## To get you started:  
yearly_species_weight <- BlackfootFish_clean |>
...
This definitely makes the reader tilt their head less! For further refinement, 
we can specify the text size for each element of the plot independently. This 
...
BlackfootFish_clean |>
  ggplot(aes(x = species, y = length, color = section)) +
  geom_boxplot() +
...
```{r arrange, message = FALSE, fig.width = 7}
library(gridExtra)

spp_weight_boxplot <- BlackfootFish_clean |> 
...
```{r arrange2, message = FALSE, fig.width = 7, fig.height=4}
...
my_plot <- ggplot(data = yearly_section_counts, 
                  mapping = aes(x = year, y = n, color = section)) +
              geom_line() +
              facet_wrap(~ species) +
              labs(title = "Observed species through time",
                    x = "Year of observation",
...
ggsave("fig/yearly_section_counts.png", my_plot, width = 15, height = 10)
...
Here, we create a list whose components are the three vectors we created in the Introduction to R chapter:
...
but there are elements in each cubby. See Figure 2.  
...
list, these can be variables, parameters, or other information. This will help you understand the structure of data.frames.
...
Year 2000 had the most fish caught.
...
## Additional Data Wrangling Material
