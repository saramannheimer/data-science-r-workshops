...
```{r challenge-7-solution, exercise.setup="okCupid-setup"}
...
> ### Challenge 8
>
>Using the `last` number from above, write an `if` statement that 
prints "You're popular!" if the number of messages exceeds 10.  
>
> Mark (Greta here) this challenge makes no sense. Is it referring to Challenge 7????


```{r challenge-8, exercise=TRUE, exercise.lines = 7, exercise.setup="okCupid-setup"}
...
> ### Challenge 9
>
>Using the if statement from Challenge 8, add the following else 
...
Now, let's take and add the corresponding entries of two vectors together!  
...
```{r challenge-10, exercise=TRUE, exercise.lines=7}
...
```{r challenge-11, eval=FALSE, exercise=TRUE, exercise.lines = 20}
BlackfootFish <- read.csv("https://raw.githubusercontent.com/saramannheimer/data-science-r-workshops/master/Intermediate%20R/AY%202020-2021/data/BlackfootFish.csv", header = TRUE)
...
```{r challenge-12, eval = FALSE, exercise=TRUE, exercise.lines = 7}
...
and `weight` columns of the `BlackfeetFish` data:  
...
We could continue to copy and paste for other columns of my data, but you can 
...
# Rewrite the code snippet from Exercise 13 to now 
...
> Start by creating a data subset with just the length and weight variables
...
BlackfootFish_subset <- data.frame(length = BlackfootFish$length, width = BlackfootFish$weight)
...
grade_result(
  pass_if(~ identical(.result,
                      data.frame(length = BlackfootFish$length, 
...
```{r challenge-18-2, exercise=TRUE, exercise.lines=7}



```

```{r challenge-18-2-solution}
...
The goal of this workshop was to teach you to write code in `R` to learn how to use relational operators, create conditional sequences, write loops and functions. The first workshop in our series contains more information on how to get started working in `R` using RStudio (see http://www.montana.edu/datascience/training/). The second workshop in our series contains information on how to write code to visualize data using `ggplot2`. The codechunks in this interactive document mimic the codechunks you can use on your own projects in RMarkdown but you will need to download and install both `R` and RStudio on your own computer. 
