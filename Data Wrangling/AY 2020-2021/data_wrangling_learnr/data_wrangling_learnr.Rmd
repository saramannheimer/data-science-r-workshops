...
source("data/setup-code-chunks.R")
```
...
```{r challenge-4-setup, eval=FALSE}
surveys_w_days <- surveys %>% 
  mutate(date = ymd(paste(year,
                          month,
                          day,
...
```{r caseWhen-setup, eval=FALSE, message=FALSE, exercise.setup="challenge-4-setup"}
surveys_days_full <- surveys_w_days %>% 
  mutate(day_of_week = case_when(day_of_week == "Mon" ~ "Monday", 
                             day_of_week == "Tue" ~ "Tuesday", 
                             day_of_week == "Wed" ~ "Wednesday", 
...
```{r relevel-setup, eval=FALSE, message=FALSE, exercise.setup="caseWhen-setup"}
...
```{r gw-setup, eval=FALSE, message=FALSE}
combined <- surveys_edited %>% 
              left_join(plots, by = "plot_id") %>%  # adding the type of plot  
              left_join(species, by = "species_id") # adding the genus, species, and taxa 
```
...
```{r pivot-wider-setup, eval=FALSE, message=FALSE, exercise.setup="gw-setup"}
surveys_gw <- combined %>%
  filter(!is.na(weight)) %>%
  group_by(plot_id, genus) %>%
  summarize(mean_weight = mean(weight))
...
```{r pivot-long-setup, eval=FALSE, message=FALSE, exercise.setup="pivot-wider-setup"}
surveys_wide <- surveys_gw %>%
  pivot_wider(names_from = genus, values_from = mean_weight)
```
...
```{r challenge-10-setup, eval=FALSE}
surveys_wide_genera <- combined %>%
  group_by(plot_id, year) %>%
  summarize(num_genera = n_distinct(genus)) %>%
  pivot_wider(names_from = year, values_from = num_genera)
...
```{r challenge-11-p-2-setup, eval=FALSE}
combined_longer <- combined %>%
  pivot_longer(cols = c(hindfoot_length, weight), names_to = "measurement",
             values_to = "values")
```
...
```{r challenge-11-p-3-setup, eval=FALSE}
combined_longer <- combined %>%
  pivot_longer(cols = c(hindfoot_length, weight), names_to = "measurement",
             values_to = "values")
...
```{r challenge-11-p-3, exercise=TRUE, exercise.lines=20, exercise.setup="challenge-11-p-3-setup"}
...
re-generate them. If you were working in RStudio, the following code would create a `data` directory inside the folder where you .Rmd exists.
...
```{r data-extract-setup, eval=FALSE, message=FALSE}
surveys_complete <- surveys_edited %>%
  filter(!is.na(weight),           # remove missing weight
         !is.na(hindfoot_length),  # remove missing hindfoot_length
         !is.na(sex))                # remove missing sex
...
```{r ready-for-export-setup, eval=FALSE}
## Extract the most common species_id
species_counts <- surveys_complete %>%
    count(species_id) %>% 
    filter(n >= 50)
