...
```{r load-data, results = 'hide', message = FALSE, exercise=TRUE}
...
>
>  Using pipes, subset the `surveys` data to include animals collected on or after
...
```{r lubridate-example, message = FALSE, exercise=TRUE, exercise.lines=20}
library(lubridate)
surveys3 <- surveys %>% 
...
Let's use `spread()` to transform `surveys3` to find the mean weight of each 
...
```{r fig.cap="Visualization of the relationship between the long to wide versions of the data using spread()."}
...
```{r fill-missing-example, exercise=TRUE}
surveys_gw %>%
