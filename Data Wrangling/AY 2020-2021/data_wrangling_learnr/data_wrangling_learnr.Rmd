...
For additional information about `lubridate` visit [the `lubridate` reference website](https://lubridate.tidyverse.org/) or look over the [`lubridate` cheatsheet](https://rawgit.com/rstudio/cheatsheets/master/lubridate.pdf). 
...
```{r caseWhen-setup, exercise.setup="challenge-4-setup"}
...
> ### Challenge 5 ![](figures/medium.jpg) {.challenge}
> 
...
```{r relevel-setup, exercise.setup="caseWhen-setup"}
...
surveys_edited %>%
  ...
  select(year, genus, species_id_weight)
```
...
```{r challenge-3-q-1, exercise=TRUE, exercise.lines = 12, exercise.eval=TRUE}
## Count Challenges:
##  1. How many animals were caught in each `plot_type` surveyed?

```

```{r challenge-3-q-2, exercise=TRUE, exercise.lines = 12, exercise.eval=TRUE}
##  2. Use `group_by()` and `summarize()` to find the mean, min, and max
## hindfoot length for each species (using `species_id`). Also add the number of
## observations (hint: see `?n`).
```

```{r challenge-3-q-3, exercise=TRUE, exercise.lines = 12, exercise.eval=TRUE}
...
```{r challenge-3-q-1-solution}
surveys %>%
        count(plot_type) 
```

```{r challenge-3-q-1-check}
grade_result(
  pass_if(~ identical(.result, surveys %>% count(plot_type) ))
)
```

```{r challenge-3-q-2-solution}
surveys %>%
    filter(!is.na(hindfoot_length)) %>%
    group_by(species_id) %>%
    summarize(
...
```{r challenge-3-q-2-check}
grade_result(
  pass_if(~ identical(.result, surveys %>%
    filter(!is.na(hindfoot_length)) %>%
    group_by(species_id) %>%
    summarize(
            mean_hindfoot_length = mean(hindfoot_length),
...
```{r load-data2, results = 'hide', message = FALSE, exercise=TRUE}
...
```{r pivot-wider-setup, exercise.setup="gw-setup"}
surveys_gw <- combined %>%
  filter(!is.na(weight)) %>%
  group_by(plot_id, genus) %>%
  summarize(mean_weight = mean(weight))
...
```{r pivot-long-setup, exercise.setup="pivot-wider-setup"}
surveys_wide <- surveys_gw %>%
  pivot_wider(names_from = genus, values_from = mean_weight)
```
...
Data Carpenters discussed how to structure data leading to the four rules defining a tidy
dataset:
...
```{r fig.cap="Visualization of tidy data"}
...
```{r fig.cap="Visualization of the relationship between the long to wide versions of the data using pivot_wider()."}
knitr::include_graphics("images/long_to_wide.jpg")
```

Let's use `pivot_wider()` to transform `surveys_edited` to find the mean weight of each 
genus in each plot over the entire survey period. We use `filter()`,
`group_by()` and `summarize()` to filter our observations and variables of 
interest, and create a new variable for the `mean_weight`. We use the pipe as 
before too.
...
```{r genus-weight-example, exercise=TRUE}
surveys_gw <- combined %>%
  filter(!is.na(weight)) %>%
  group_by(plot_id, genus) %>%
  summarize(mean_weight = mean(weight))
...
```{r gather-example, exercise=TRUE}
surveys_long <- surveys_wide %>%
  pivot_longer(cols=-plot_id, names_to = "genus", values_to = "mean_weight")
...
```{r fig.cap="Visualization of the relationship between the wide to long versions of the data using gather()."}
knitr::include_graphics("images/wide_to_long.jpg")
```
...
```{r spread-example2, exercise=TRUE}
surveys_wide %>%
...
>
> 1. Spread the `surveys_edited` data frame with `year` as columns, `plot_id` 
...
>
> 
> 3. The `surveys_edited` data set has two measurement columns: `hindfoot_length` and
>    `weight`.  This makes it difficult to do things like look at the 
>    relationship between mean values of each measurement per year in different
>    plot types. Let's walk through a common solution for this type of problem.
