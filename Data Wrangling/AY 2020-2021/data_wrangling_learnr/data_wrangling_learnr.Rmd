...
```{r load-data, results = 'hide', message = TRUE, exercise=TRUE}
...
```{r inspect-data, results = 'hide', exercise=TRUE}
## inspect the data
glimpse(surveys)
```
...
```{r echo=TRUE, eval=TRUE, context="server"}
output$surveyData <- renderTable({ head(surveys, n=10) })
```
...
<!-- Stuck here... how to make this a challenge and to get the answers out from what it used to be... -->
...
names(surveys3$day_of_week)
```


```{r challenge-4-check}
grade_result(
  pass_if(~ identical(.result, levels(surveys3$day_of_week)))
)
```
...
```{r challenge-5, exercise=TRUE, exercise.eval=TRUE}
...
```{r weekend-dates-example, exercise=TRUE, exercise.setup="lubridate-example"}
...
### Joining `surveys3` Data  

To join the `surveys3` data with the `plots` data and `species` data, we will need
to join statements. As we are interested in adding this information to our 
already existing data frame, `surveys3`, a left join is the most appropriate.  
...
In `surveys3` , the rows of `surveys3` contain the values of variables associated
with each record (the unit), values such as the weight or sex of each animal 
associated with each record. What if instead of comparing records, we 
wanted to compare the different mean weight of each genus between plots? 
(Ignoring `plot_type` for simplicity).
...
Let's use `pivot_wider()` to transform `surveys3` to find the mean weight of each 
genus in each plot over the entire survey period. We use `filter()`,
`group_by()` and `summarize()` to filter our observations and variables of 
interest, and create a new variable for the `mean_weight`. We use the pipe as 
before too.
...
>
> 1. Spread the `surveys3` data frame with `year` as columns, `plot_id` 
...
>
> 
> 3. The `surveys3` data set has two measurement columns: `hindfoot_length` and
>    `weight`.  This makes it difficult to do things like look at the 
>    relationship between mean values of each measurement per year in different
>    plot types. Let's walk through a common solution for this type of problem.
