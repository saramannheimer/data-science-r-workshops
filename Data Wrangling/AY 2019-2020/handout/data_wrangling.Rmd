...
> commands from the **`tidyr`** package.
...
**`dplyr`** is a package for making tabular data manipulation easier. It pairs 
nicely with **`tidyr`** which enables you to swiftly convert between different 
data formats for plotting and analysis.
...
ADDITIONAL INFORMATION ON FILTERING SHOULD GO HERE!
...
```{r}
## Now take that data frame, and make it long again, by (re)pivoting on the 
## year columns. 


```

> 3. The `combined` data set has two measurement columns: `hindfoot_length` and
