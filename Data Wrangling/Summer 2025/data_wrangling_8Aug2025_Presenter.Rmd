...
subtitle: "Data Science Workshop - Session 4"
author: "Data Carpentry contributors & Montana State University R Workshops Team"
output: pdf_document
header-includes: |
  \usepackage{fancyhdr}
  \pagestyle{fancy}
  \fancyfoot[LO,RE]{Session 4: Data Wrangling - Presenter}
...
```{r challenge-2-solution, include=TRUE}
...
```{r challenge-7-2-solution, include=TRUE}
surveys_edited %>%
  filter(!is.na(weight)) %>%
  group_by(year) %>%
...
```{r challenge-8-solution, include=TRUE}
surveys_edited %>%
        count(plot_id) 
```
...
```{r challenge-9-solution, include=TRUE}
...
```{r challenge-10-solution, include=TRUE}
...
```{r challenge-11-p-1-solution, include=TRUE}
...
```{r challenge-11-p-2-solution, include=TRUE}
...
```{r challenge-11-p-3-solution, include=TRUE}
...
The goal of this workshop was to familiarize you with RStudio and the `R` language, and teach you to write code in `R` for data visualization and data wrangling in a reproducible fashion. Recordings of the previous workshops are available at http://www.montana.edu/datascience/training/. 


Other workshops available via YouTube include Intermediate R, which explores 
more sophisticated R code involving logicals, loops, and functions, and
Introduction to Python which works through the same material as Introduction to
`R` but in Python.
...
CC-BY. ![Creative Commons License](images/88x31.png) This work is licensed under a 
