---
title: "Wrangling, Analyzing, and Exporting Data with the Tidyverse"
author: "Authors: Data Carpentry contributors & MSU R Workshop Leadership Team"
...
Whereas, with data wrangling, all of these processes are done, but in a 
__reproducible__ manner, such as using an `R` script!

Packages in `R` are sets of additional functions that lets you do more
...
"umbrella-package" that installs several packages useful for data analysis that
...
in-memory and thus the amount of data you can work with is limited by the available
...
The last option, *pipes*, is a more recent addition to `R`. Pipes let you take
...
The order of the levels chosen by `R` may not be what we wanted, but we can 
...
which you wish to add additional information to (the contents of `y`). This join will 
...
```{r out.width="70%"}
knitr::include_graphics("images/pivot_wider_graphic.png", dpi=NA)
```
...
```{r challenge-9, exercise=TRUE, exercise.lines=10}
...
The opposing situation could have occurred if we had been provided with data in the
...
```{r out.width="70%"}
knitr::include_graphics("images/pivot_longer_graphic.png", dpi=NA)
```
...
```{r pivot-long-example2, exercise=TRUE}
surveys_wide %>%
...
```{r challenge-11-p-1, exercise=TRUE, exercise.lines=10}
...
```{r challenge-11-p-2, exercise=TRUE, exercise.lines=20}
...
Let's start by removing observations of animals for which `weight` or 
`hindfoot_length` is missing, or the `sex` has not been determined:
...
```{r data-extract-exercise, exercise=TRUE, exercise.lines=12}
...
The goal of this workshop was to teach you to write code in `R` to perform data wrangling in a reproducible fashion. Recordings of the previous workshops are available at http://www.montana.edu/datascience/training/. We plan to offer this same series in the spring (dates to be determined) so these web-interface "Shiny" apps will continue to evolve and links might change. If they are not available when you try to revisit them, please contact   one of the authors and we can point you to the current versions of them. 
