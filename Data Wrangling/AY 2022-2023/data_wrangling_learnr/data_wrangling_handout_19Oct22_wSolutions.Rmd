...
header-includes: |
  \usepackage{fancyhdr}
  \pagestyle{fancy}
  \fancyfoot[LO,RE]{Data Wrangling}
  \fancyfoot[CO,CE]{November 17, 2021}
...
```{r challenge-1-solution, include=TRUE}
surveys %>%
     filter(year >= 2001) %>%
...
```{r  challenge-4-solution, include=TRUE}
...
```{r challenge-5-solution, include=TRUE}
...
```{r challenge-6-solution, include=TRUE}
...
```{r challenge-7-1-solution, include=TRUE}
surveys_edited %>%
  filter(!is.na(hindfoot_length)) %>%
  group_by(species_id) %>%
